...
_INLINE_SYSTEM_PROMPT = (
    "You are the Systems Architect for ZTAMP — a multi-agent cloud infrastructure "
    "orchestration engine. Design precise, step-by-step executable workflows using the "
    "defined block types (action_block, conditional_block, code_block, notification_block, "
    "result_block, parameter_block). Output valid JSON only. Full instructions are in "
    "backend/prompts/planner_agent.md."
)


# ─── Template Injection Prompt (loaded from .md) ────────

_TEMPLATE_INJECTION_CACHE: str | None = None


def _get_template_injection_prompt() -> str:
    """
    Load the template injection instructions from template_injection.md.
    Falls back to a minimal inline default if the file is unavailable.
    """
    global _TEMPLATE_INJECTION_CACHE

    if _TEMPLATE_INJECTION_CACHE is not None:
        return _TEMPLATE_INJECTION_CACHE

    try:
        from prompt_loader import read_prompt_file
        _TEMPLATE_INJECTION_CACHE = read_prompt_file("template_injection.md")
        logger.info(
            "Template injection prompt loaded from .md (%d chars)",
            len(_TEMPLATE_INJECTION_CACHE),
        )
    except Exception as e:
        logger.warning(
            "Failed to load template_injection.md: %s — using inline fallback", e
        )
        _TEMPLATE_INJECTION_CACHE = _INLINE_TEMPLATE_INJECTION

    return _TEMPLATE_INJECTION_CACHE


_INLINE_TEMPLATE_INJECTION = """
## ⚠️ WORKFLOW TEMPLATE PROVIDED — STRICT BLUEPRINT MODE
The user provided a YAML Workflow Template. Treat it as the absolute source of truth.
Map inputs to parameter_block, follow steps chronologically, wire variables with {{}} syntax.
Do NOT hallucinate steps. Generate ONLY what the template defines.

### USER TEMPLATE INSTRUCTIONS:
```yaml
{template_content}
```
You MUST generate a workflow that EXACTLY follows this template.
"""
...
class PlannerAgent:
    ...
    async def process_message(
        ...
        if template_content and template_content.strip():
            template_block = _get_template_injection_prompt().replace(
