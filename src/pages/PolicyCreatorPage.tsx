...
import { usePolicies, type PolicyCategory, type PolicyTarget, type AccessRestrictionConfig, categoryLabels, categoryIcons, categoryColors } from '../context/PolicyContext';
import { colors, shadows, fonts } from '../theme';
import StepAccessRestriction from '../components/policy/StepAccessRestriction';

/* ═══════════════════════════════════════════════════════
   TYPES
   ═══════════════════════════════════════════════════════ */

type CreatorStep = 'define' | 'configure' | 'review' | 'generated';
...
const defaultAccessRestriction: AccessRestrictionConfig = {
    whitelistedSites: [],
    enableTimeBasedAccess: false,
    enableDataEgressLimits: false,
...
const defaultForm: PolicyFormData = {
    ...
    accessRestriction: { ...defaultAccessRestriction },
};
...
function generateRegoCode(form: PolicyFormData): string {
    ...
    // ─── ACCESS RESTRICTION: URL Whitelist Rego ───
    if (form.category === 'access' && form.accessRestriction.whitelistedSites.length > 0) {
        const configuredSites = form.accessRestriction.whitelistedSites.filter(e => e.selectedAccessMode);
        if (configuredSites.length > 0) {
            rego += `# ═══ URL Whitelist for Planner Agent ═══\n`;
            rego += `# Only whitelisted domains are accessible.\n\n`;

            // Whitelist set
            rego += `whitelisted_domains := {${configuredSites.map((s) => `"${s.domain}"`).join(', ')}}\n\n`;

            // Access modes per domain
            rego += `# Access modes per domain\n`;
            for (const site of configuredSites) {
                rego += `access_mode["${site.domain}"] := "${site.selectedAccessMode}"\n`;
            }
            rego += `\n`;

            // Allow whitelisted
            rego += `# Allow access to whitelisted domains\n`;
            rego += `allow {\n    input.agent.name == "planner_agent"\n    input.destination.host in whitelisted_domains\n    input.access.method == access_mode[input.destination.host]\n}\n\n`;

            // Deny non-whitelisted
            rego += `# Deny all non-whitelisted URLs\n`;
            rego += `deny[msg] {\n    input.agent.name == "planner_agent"\n    not input.destination.host in whitelisted_domains\n    msg := sprintf("URL '%s' is not whitelisted for Planner Agent", [input.destination.host])\n}\n\n`;

            // Auth-required domains
            const authSites = configuredSites.filter(s => s.scanResult?.requiresAuth);
            if (authSites.length > 0) {
                rego += `# ─── Authenticated Domains ───\n`;
                rego += `auth_required_domains := {${authSites.map(s => `"${s.domain}"`).join(', ')}}\n\n`;
                rego += `deny[msg] {\n    input.destination.host in auth_required_domains\n    not input.user.credentials[input.destination.host]\n    msg := sprintf("Domain '%s' requires authentication — no credentials found for user '%s'",\n        [input.destination.host, input.user.name])\n}\n\n`;
            }
        }

        // Time-based access
        if (form.accessRestriction.enableTimeBasedAccess) {
            const start = form.accessRestriction.timeWindowStart || '09:00';
            const end = form.accessRestriction.timeWindowEnd || '18:00';
            rego += `# ─── Time-Based Access Window ───\n`;
            rego += `time_window_start := "${start}"\n`;
            rego += `time_window_end := "${end}"\n`;
            rego += `time_zone := "${form.accessRestriction.timeZone || 'UTC'}"\n\n`;
            rego += `deny[msg] {\n    input.agent.name == "planner_agent"\n    current_time := time.clock([time.now_ns(), time_zone])\n    current_hour_min := sprintf("%02d:%02d", [current_time[0], current_time[1]])\n    current_hour_min < time_window_start\n    msg := sprintf("Internet access is restricted before %s %s", [time_window_start, time_zone])\n}\n\n`;
            rego += `deny[msg] {\n    input.agent.name == "planner_agent"\n    current_time := time.clock([time.now_ns(), time_zone])\n    current_hour_min := sprintf("%02d:%02d", [current_time[0], current_time[1]])\n    current_hour_min > time_window_end\n    msg := sprintf("Internet access is restricted after %s %s", [time_window_end, time_zone])\n}\n\n`;
        }

        // Data egress limits
        if (form.accessRestriction.enableDataEgressLimits) {
            const maxMb = form.accessRestriction.maxEgressMbPerDay || 500;
            rego += `# ─── Data Egress Limits ───\n`;
            rego += `max_egress_mb_per_day := ${maxMb}\n\n`;
            rego += `deny[msg] {\n    input.agent.name == "planner_agent"\n    input.agent.egress_mb_today > max_egress_mb_per_day\n    msg := sprintf("Data egress limit exceeded: %d/%d MB per day",\n        [input.agent.egress_mb_today, max_egress_mb_per_day])\n}\n\n`;
        }

        // Session binding
        if (form.accessRestriction.enableSessionBinding) {
            rego += `# ─── User Session Binding ───\n`;
            rego += `deny[msg] {\n    input.agent.name == "planner_agent"\n    not input.session.active\n    msg := "Agent internet access requires an active user session"\n}\n\n`;
            rego += `deny[msg] {\n    input.agent.name == "planner_agent"\n    input.session.user != input.request.initiating_user\n    msg := "Agent session is bound to a different user"\n}\n\n`;
        }
    } else {
        // ─── STANDARD PERMISSIONS (non-access policies) ───
...
function generateJsonPolicy(form: PolicyFormData): object {
    ...
    if (form.category === 'access') {
        // Access Restriction policy — include whitelist and advanced controls
        (base.spec as Record<string, unknown>).accessRestriction = {
            whitelistedSites: form.accessRestriction.whitelistedSites
                .filter(e => e.selectedAccessMode)
                .map(e => ({
                    domain: e.domain,
                    accessMode: e.selectedAccessMode,
                    requiresAuth: e.scanResult?.requiresAuth || false,
                    authType: e.scanResult?.authType || null,
                    apiEndpoint: e.scanResult?.apiEndpoint || null,
                    mcpServer: e.scanResult?.mcpServer || null,
                })),
            timeBasedAccess: form.accessRestriction.enableTimeBasedAccess ? {
                start: form.accessRestriction.timeWindowStart || '09:00',
                end: form.accessRestriction.timeWindowEnd || '18:00',
                timeZone: form.accessRestriction.timeZone || 'UTC',
            } : null,
            dataEgressLimits: form.accessRestriction.enableDataEgressLimits ? {
                maxMbPerDay: form.accessRestriction.maxEgressMbPerDay || 500,
            } : null,
            sessionBinding: form.accessRestriction.enableSessionBinding,
        };
    } else {
        // Standard policy — include permissions, sandbox, etc.
        (base.spec as Record<string, unknown>).sandbox = {
            mode: form.sandboxMode,
            maxExecutionTime: `${form.maxExecutionTime}s`,
            maxMemory: form.maxMemory,
            maxRetries: form.maxRetries,
        };
        (base.spec as Record<string, unknown>).permissions = {
...
function getSteps(category: PolicyCategory): { key: CreatorStep; label: string; icon: string }[] {
    return [
        { key: 'define', label: 'Define Policy', icon: 'edit' },
        {
            key: 'configure',
            label: category === 'access' ? 'Access Restriction' : 'Permissions & Limits',
            icon: category === 'access' ? 'shield' : 'toggle_on',
        },
...
export default function PolicyCreatorPage() {
    ...
    const steps = useMemo(() => getSteps(form.category), [form.category]);
    const stepIndex = steps.findIndex((s) => s.key === currentStep);

    const canProceed = useMemo(() => {
        if (currentStep === 'define') return form.name.trim().length > 0 && form.targets.length > 0;
        if (currentStep === 'configure' && form.category === 'access') {
            // Access restriction: at least one site must be fully configured
            return form.accessRestriction.whitelistedSites.some(e => e.selectedAccessMode !== null);
        }
        return true;
    }, [currentStep, form]);

    const handleNext = useCallback(() => {
        if (currentStep === 'define') setCurrentStep('configure');
        else if (currentStep === 'configure') setCurrentStep('review');
    }, [currentStep]);

    const handleBack = useCallback(() => {
        if (currentStep === 'configure') setCurrentStep('define');
    ...
    const handleCreate = useCallback(() => {
        ...
        addPolicy({
            ...
            severity: form.severity,
            ...(form.category === 'access' ? { accessRestriction: form.accessRestriction } : {}),
        });
        setCreated(true);
    }, [form, addPolicy]);
                ...
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ textAlign: 'center', maxWidth: 480 }}>
                        ...
                        <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                            <button style={st.primaryBtn} onClick={() => { setForm({ ...defaultForm, permissions: defaultPermissions.map((p) => ({ ...p })), accessRestriction: { ...defaultAccessRestriction } }); setCreated(false); setCurrentStep('define'); }}>
    ...
    return (
        <div style={st.page(isDark)}>
            ...
            <div style={st.body}>
                ...
                {currentStep === 'configure' && form.category === 'access' && (
                    <StepAccessRestriction
                        config={form.accessRestriction}
                        setConfig={(config) => setForm((f) => ({ ...f, accessRestriction: config }))}
                        targets={form.targets}
                        isDark={isDark}
                    />
                )}
                {currentStep === 'configure' && form.category !== 'access' && (
                    <StepPermissions form={form} setForm={setForm} isDark={isDark} />
                )}
