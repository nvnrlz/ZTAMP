...
function FileLibraryTab() {
    ...
    // File selection state (declared early so handleVectorizeSelected can reference it)
    ...
    const handleFiles = useCallback((incoming: FileList | null) => {
        ...
        for (let i = 0; i < incoming.length; i++) {
            ...
            if (f.size > MAX_FILE_SIZE) {
                setSizeError(`"${f.name}" exceeds the 1 GB limit (${formatBytes(f.size)})`);
                continue;
            }

            // Upload to backend with scope query parameter
            // If file came from folder picker, webkitRelativePath includes the relative path
            const formData = new FormData();
            formData.append('file', f);

            const relPath = (f as any).webkitRelativePath || '';
            const params = new URLSearchParams({ scope: uploadScope });
            if (relPath) params.set('relativePath', relPath);

            fetch(`http://localhost:4000/api/upload?${params.toString()}`, {
                method: 'POST',
                body: formData,
            })
                .then((res) => res.json())
    ...
    const handleVectorizeSelected = useCallback(() => {
        if (vecJobId) return; // already running
        if (selectedFileIds.size === 0) return;

        const scopedFiles = getFilesByScope(activeScope);
        const uploadedPaths = scopedFiles
            .filter((f) => selectedFileIds.has(f.id) && f.status === 'uploaded')
            .map((f) => f.relativePath || f.name);
        if (uploadedPaths.length === 0) return;

        setVecProgress({ status: 'starting', total: uploadedPaths.length, completed: 0, currentFile: null });

        fetch('http://localhost:4000/api/vectorize', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ files: uploadedPaths, scope: activeScope }),
        })
            .then((res) => res.json())
            .then((data) => {
                if (data.jobId) {
                    ...
                    setSelectedFileIds(new Set()); // clear selection on start
                } else {
                    setVecProgress(null);
                }
            })
    ...
    }, [activeScope, getFilesByScope, vecJobId, selectedFileIds]);
    ...
    const handleDelete = useCallback((file: LibraryFile) => {
        ...
        if (file.relativePath) params.set('relativePath', file.relativePath);
        fetch(`http://localhost:4000/api/files/${encodeURIComponent(file.name)}?${params.toString()}`, {
            method: 'DELETE',
        })
            .then(() => refreshFiles())
            .catch(() => { });
    ...
    const toggleSelectAll = () => {
        const allIds = scopedFiles.map(f => f.id);
    ...
    const selectedUploadedCount = scopedFiles.filter(
        f => selectedFileIds.has(f.id) && f.status === 'uploaded'
    ).length;
    const allScopedSelected = scopedFiles.length > 0 && scopedFiles.every(f => selectedFileIds.has(f.id));

    // Group files: root-level files (no relativePath) + folder-grouped files
    const rootFiles = scopedFiles.filter(f => !f.relativePath);
    const folderMap = new Map<string, LibraryFile[]>();
    for (const f of scopedFiles) {
        if (f.relativePath && f.relativePath.includes('/')) {
            // Group by the full parent directory path (e.g. "aws_docs/DynamoDB")
            const parts = f.relativePath.split('/');
            const parentDir = parts.slice(0, -1).join('/');
            if (!folderMap.has(parentDir)) folderMap.set(parentDir, []);
            folderMap.get(parentDir)!.push(f);
        } else if (!f.relativePath) {
            // already in rootFiles
        } else {
            // single-segment relative path (file at root of scope with no subdirectory)
            rootFiles.push(f);
        }
    }
        ...
        <div style={contentContainer}>
            ...
            <div style={infoBox(isDark)}>
                ...
                <div>
                    ...
                    Max file size: <strong>1 GB</strong>.
                </div>
            </div>
                ...
                <div style={cardBody}>
                    ...
                    >
                        ...
                        <div style={dropTitle(isDark)}>
                            {dragActive ? 'Drop files or folders here' : 'Drag & drop your documents or folders here'}
                        </div>
                        <div style={dropSub(isDark)}>or click to browse · max 1 GB per file</div>
                        ...
                        />
                        {/* Folder input — webkitdirectory enables directory selection */}
                        <input
                            ref={folderInputRef}
                            type="file"
                            multiple
            ...
            <div style={card(isDark)}>
                <div style={cardHeader(isDark)}>
                    ...
                    {/* Selection action bar — replaces Vectorize All */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        {vecProgress && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500, color: isDark ? '#93c5fd' : '#2563eb' }}>
                        ...
                        {selectedFileIds.size > 0 && !vecProgress && (
                            <>
                                <span style={{
                                    fontSize: 12,
                                    fontWeight: 500,
                                ...
                                {selectedUploadedCount > 0 && (
                                    <button
                                        style={vectorizeAllBtn(isDark)}
                                        onClick={handleVectorizeSelected}
                                    >
                                        <span className="material-icons" style={{ fontSize: 16 }}>memory</span>
                                        Vectorize {selectedUploadedCount === 1 ? '1 File' : `${selectedUploadedCount} Files`}
                                    </button>
                                )}
                            </>
                        )}
                ...
                {loading ? (
                    ...
                    <table style={fileTable(isDark)}>
                        <thead>
                            <tr>
                                <th style={{ ...thStyle(isDark), width: 36, textAlign: 'center' as const }}>
                                    <input
                                        type="checkbox"
                                        checked={allScopedSelected}
                                        onChange={toggleSelectAll}
                                        style={{ cursor: 'pointer', accentColor: colors.primary }}
                                        title={allScopedSelected ? 'Deselect all' : 'Select all'}
                                    />
                                </th>
                        ...
                        <tbody>
                            ...
                            {sortedFolders.map(folder => {
                                ...
                                const allFolderSelected = folderFiles.every(f => selectedFileIds.has(f.id));
                                const someFolderSelected = folderFiles.some(f => selectedFileIds.has(f.id));
                                return (
                                    <>
                                        {/* Folder header row */}
                                        <tr
                                        ...
                                        >
                                            <td style={{
                                                ...
                                                <input
                                                    ...
                                                    style={{ cursor: 'pointer', accentColor: colors.primary }}
                                                    title={allFolderSelected ? 'Deselect folder' : 'Select all in folder'}
                                                />
                                            </td>
                                            <td
                                                colSpan={5}
                                            ...
                                            >
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    ...
                                                    }}>{isCollapsed ? 'folder' : 'folder_open'}</span>
                                                    <span style={{
                                                        fontWeight: 600,
                                                        fontSize: 13,
                                                        color: isDark ? '#e5e7eb' : '#1f2937',
                                        ...
                                        {/* Folder files (collapsible) */}
                                        {!isCollapsed && folderFiles.map((f: LibraryFile) => {
                                            const fi = fileIcon(f.name);
                                            return (
                                                <tr key={f.id}>
                                                    <td style={{ ...tdStyle(isDark), width: 36, textAlign: 'center' as const }}>
                                                        <input
                                                            type="checkbox"
