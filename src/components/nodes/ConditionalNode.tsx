...
/**
 * B. Conditional Block — Yellow Rectangle
 * Shows: logical operator badge + number of active rules
 */
...
const iconBadge: CSSProperties = {
    ...
    height: 20,
    borderRadius: '50%',
    backgroundColor: colors.nodeYellow,
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
...
const operatorBadge: CSSProperties = {
    fontSize: 8,
    fontWeight: 700,
    color: '#78350f',
...
const ruleCountTag = (isDark: boolean): CSSProperties => ({
    fontSize: 8,
    fontWeight: 500,
...
const handleStyle = { background: colors.nodeYellow, width: 8, height: 8 };

export default function ConditionalNode({ data, id }: NodeProps) {
    const { isDark } = useTheme();
    const { nodeConfigs } = useWorkflow();
    const d = data as WorkflowNodeData;
    const state = d.liveState || 'idle';
    const config = nodeConfigs[id]?.conditionalConfig;
    ...
    return (
        <div style={nodeStyle(isDark, state)}>
            <div style={iconBadge}>
                <span className="material-icons" style={{ fontSize: 12, color: '#78350f' }}>call_split</span>
            </div>
            <Handle type="target" position={Position.Left} style={handleStyle} />
            <Handle type="target" position={Position.Top} id="top" style={handleStyle} />
            <span style={labelStyle(isDark)}>{d.label}</span>
            <span style={operatorBadge}>{operator}</span>
            <span style={ruleCountTag(isDark)}>
